...
public class PassProcess extends GameProcess {
	
	private static PassProcess instance;
	
	public static PassProcess getInstance(){
		if (instance == null)
			instance = new PassProcess();
		return instance;
	}
	...
	private void continuePass(GameState state, int result, boolean execute) throws IllegalActionException {
		...
		if (result >= success){
			...
			CatchProcess.getInstance().run(state, null, null);
		...
		if (result == 1){
			...
			state.getPitch().getBall().setSquare(catcher.getPosition());
			ScatterPassProcess.getInstance().run(state, null, null);
	...
	private void scatterPass(GameState state) throws IllegalActionException {
		...
		if (!state.getPitch().isBallInsidePitch()){
			...
			ThrowInProcess.getInstance().run(state, null, null);
