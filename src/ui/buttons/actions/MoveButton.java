...
public class MoveButton extends ActionButton {
	...
	public MoveButton(int origX, int origY, BufferedImage unavaiableImage, BufferedImage avaiableImage, BufferedImage avaiableImageHover, BufferedImage activeImage, BloodBowlUI ui, boolean active) {
	...
	public void checkLayerActivation(GameState state, BloodBowlUI ui) {
		...
		if (ui.getSelectedPlayer() == null){
			...
			available();
		} else if (ui.getSelectedPlayer().getPlayerStatus().getTurn() == PlayerTurn.MOVE_ACTION){
			active();
		}
