...
public class HandoffButton extends ActionButton {
	
	public HandoffButton(int origX, int origY, BufferedImage unavaiableImage, BufferedImage avaiableImage, BufferedImage avaiableImageHover, BufferedImage activeImage, BloodBowlUI ui, boolean active) {
	...
	public void checkLayerActivation(GameState state, BloodBowlUI ui) {
		...
		if (ui.getSelectedPlayer() == null){
			...
			unavailable();
				...
				!state.getMovingTeam().getTeamStatus().hasHandedOf()){
			available();
		} else if (ui.getSelectedPlayer().getPlayerStatus().getTurn() == PlayerTurn.HAND_OFF_ACTION){
			active();
		}
