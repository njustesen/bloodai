...
public class BlitzButton extends ActionButton {
	...
	public BlitzButton(int origX, int origY, BufferedImage unavaiableImage, BufferedImage avaiableImage, BufferedImage avaiableImageHover, BufferedImage activeImage, BloodBowlUI ui, boolean active) {
	...
	public void checkLayerActivation(GameState state, BloodBowlUI ui) {
		...
		if (ui.getSelectedPlayer() == null){
			...
			unavailable();
					...
					!state.getMovingTeam().getTeamStatus().hasBlitzed()){
			available();
		} else if (ui.getSelectedPlayer().getPlayerStatus().getTurn() == PlayerTurn.BLITZ_ACTION){
			active();
		}
